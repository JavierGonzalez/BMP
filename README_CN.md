...
1. 您的地址可在您当前挖矿币种的最后`4,032`个区块爆块输出里面找到，币种可以是BTC、BCH或者BSV。
...
- **power_by_action** 
 正在开发中。为了实现全面灵活性，BMP将允许通过非爆块BMP协议操作，向一个或多个地址委托某百分比的算力。同样，此协议将允许你修改或撤销该算力，并从下一个爆块区块起立即生效。
...
- 矿工的地址可在您当前挖矿币种的最后`4,032` 个区块爆块输出里面找到，币种可以是BTC、BCH或者BSV。
